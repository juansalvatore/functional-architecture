...
var sum_ = xs => List(xs).reduce((acc, x) => acc + x, 0);
const sum__ = xs =>
  List(xs)
    .map(Sum)
...
var anyLessThanZero_ = xs =>
  List(xs).reduce((acc, x) => (acc < 0 ? true : false), false);
const anyLessThanZero__ = xs =>
  List(xs).foldMap((acc, x) => (acc < 0 ? Any(true) : Any(false)), Any.empty());

const anyLessThanZero = xs => List(xs).foldMap(x => Any(x < 0), Any.empty());
...
var max_ = xs => List(xs).reduce((acc, x) => (acc > x ? acc : x), -Infinity);

const max = xs => List(xs).foldMap(Max, Max.empty());
